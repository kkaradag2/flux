...
test('Organizer executor with real JSONL transport and mock App Server',async t=>{
 ...
 const task={key:'build',title:'Build',description:'Implement signup',ownerAgentId:'dev',dependsOn:[],acceptanceCriteria:['Email is validated']};
 ...
 for(const output of ['SECRET_RAW markdown','```json\n'+JSON.stringify(decisions[0])+'\n```',JSON.stringify({...decisions[2],tasks:[{...task,ownerAgentId:'unknown'}]}),JSON.stringify({...decisions[2],tasks:[{...task,ownerAgentId:'off'}]}),JSON.stringify({...decisions[2],tasks:[{...task,dependsOn:['missing']}]} )])await t.test('invalid structured decision rejected without leaking model output',async()=>{const f=fixture(output);await assert.rejects(new Executor(f.source).execute(input),e=>e.code==='INVALID_STRUCTURED_RESULT'&&!e.message.includes('SECRET_RAW')&&!JSON.stringify(e).includes('SECRET_STDERR'));f.clean()});
