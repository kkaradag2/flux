...
export type ChatSessionOptions = { cwd: string; instructions: string; runtime: AgentRuntime; threadId?: string | null; onThread?: (threadId: string) => Promise<void> };
    ...
    try {
      ...
      if (!this.threadId) {
        ...
        const config: Omit<AppServerRequests['thread/start'], 'ephemeral'> = {
          cwd: options.cwd, approvalPolicy: 'never', sandbox: 'read-only',
          developerInstructions: options.instructions,
          ...(options.runtime.model ? { model: options.runtime.model } : {}),
          config: { web_search: 'disabled', mcp_servers: mcpServers, plugins, 'features.apps': false, 'features.multi_agent': false },
        };
        ...
        } else response = await wait(wire.request('thread/start', { ...config, ephemeral: false }));
        const started = object(response);
        this.threadId = identifier(object(started.thread).id);
        if (options.threadId && this.threadId !== options.threadId) throw new ChatThreadUnavailableError();
        if (started.approvalPolicy !== 'never' || object(started.sandbox).type !== 'readOnly' || object(started.sandbox).networkAccess === true) throw new SmokeTestError('PROTOCOL_ERROR');
        await wait(options.onThread?.(this.threadId) ?? Promise.resolve());
