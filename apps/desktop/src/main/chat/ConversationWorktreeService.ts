...
export class WorktreeError extends Error {
  constructor(public readonly code: 'BASE_BRANCH_UNAVAILABLE' | 'WORKTREE_FAILED' | 'WORKTREE_MISSING') {
...
export interface ConversationWorktrees {
  ensure(record: Conversation, projectPath: string, save: () => Promise<void>): Promise<string>;
  inspect(record: Conversation, projectPath: string): Promise<boolean>;
}
...
export class ConversationWorktreeService implements ConversationWorktrees {
  ...
  constructor(private root: string, private git = new GitCommandRunner(), private repositories = new GitRepositoryService(git)) {}
  ...
  private async location(record: Conversation, repository: string): Promise<{ directory: string; branch: string }> {
    const id = conversationId(record.id);
    if (!/^[a-zA-Z0-9_-]{1,100}$/.test(record.projectId)) throw new WorktreeError('WORKTREE_FAILED');
    ...
    if (within(project, requestedRoot)) throw new WorktreeError('WORKTREE_FAILED');
    await mkdir(requestedRoot, { recursive: true });
    if ((await lstat(requestedRoot)).isSymbolicLink()) throw new WorktreeError('WORKTREE_FAILED');
    ...
    if (within(project, root)) throw new WorktreeError('WORKTREE_FAILED');
    const parent = path.join(root, record.projectId), directory = path.join(parent, id);
    await mkdir(parent, { recursive: true });
    if ((await lstat(parent)).isSymbolicLink() || pathKey(await realpath(parent)) !== pathKey(parent)) throw new WorktreeError('WORKTREE_FAILED');
  ...
  async inspect(record: Conversation, repository: string): Promise<boolean> {
  ...
  async ensure(record: Conversation, repository: string, save: () => Promise<void>): Promise<string> {
    try {
      ...
      return await this.locked(repository, async () => {
        ...
        if (await exists(directory) || (await this.repositories.getLocalBranches(repository)).includes(branch)) throw new WorktreeError('WORKTREE_FAILED');
        ...
        try {
          ...
          if (ownedBranch) await this.cleanup(repository, directory, branch, oid);
