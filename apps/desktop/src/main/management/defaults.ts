...
import { leadInstructions } from './leadInstructions';
export function defaultAgents(): AgentDefinition[] {
 const now = new Date().toISOString();
 const rows = [
 { id: 'lead', name: 'Lead', description: 'Plans the request and coordinates the team.', icon: 'robot', lines: [] },
 ...
 return rows.map(row => ({ id: row.id, name: row.name, description: row.description, avatar: { type: 'builtin', value: row.icon }, runtime: { type: 'codex', model: null, reasoningEffort: 'default' }, instructionsMarkdown: row.id === 'lead' ? leadInstructions : '# Responsibilities\n\n' + row.lines.map(line => '- ' + line).join('\n'), enabled: true, createdAt: now, updatedAt: now }));
}
