...
export function CodexRuntimeRow({ snapshot, onRetry, onUpdate, onChoose }: { snapshot: CodexRuntimeSnapshot; onRetry: () => void; onUpdate: () => void; onChoose: () => void }) {
  ...
  return <section className="runtime-health" aria-labelledby="codex-runtime-title" aria-busy={view.busy}>
    <header><div><h3 id="codex-runtime-title">Codex</h3><span className={'runtime-status runtime-status--' + view.tone} role="status"><span aria-hidden="true" />{view.label}</span></div>
      <div className="runtime-actions">
        ...
        {view.canUpdate && <button className="agent-button agent-button-primary" disabled={view.busy} onClick={onUpdate}>{view.updating ? 'Updating…' : 'Update Codex'}</button>}
    ...
    <p className="runtime-message">{view.message}</p>
