...
export function ChatMessageList({ messages, orchestration, planningName, onExecute, onStop, onRetry, onAskOrganizer, onContinueAttention }: { messages: readonly ChatMessage[]; orchestration?: ConversationOrchestrationView | null; planningName?: string | null; onExecute?: () => void; onStop?: () => void; onAskOrganizer?: (() => void) | undefined; onContinueAttention?: (() => void) | undefined; onRetry?: () => void }) {
  ...
  return <div ref={list} className="chat-message-list" role="log" aria-label="Chat messages" aria-live="polite"><div className="chat-message-content">{messages.map(message => (
    <article className={message.role === 'user' ? 'user-message' : message.role === 'error' ? 'chat-error' : 'agent-message'} key={message.id}>
      {message.role === 'user' ? <><span className="message-author">You</span><p>{message.text}</p></> : message.role === 'error' ? <p>{message.text}</p> : <>
        ...
        {message.planRunId && orchestration?.run?.id === message.planRunId && <ExecutionPlanCard view={orchestration} onExecute={onExecute} onStop={onStop} onRetry={onRetry} onAskOrganizer={onAskOrganizer} onContinueAttention={onContinueAttention} />}
