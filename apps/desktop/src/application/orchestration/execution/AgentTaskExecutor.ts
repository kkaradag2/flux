...
export type TaskExecutionContext = Readonly<{ run: TeamRun; plan: ExecutionPlan; task: AgentTask; runtimeIdentity?: { sourceId: string; version: string }; agent: PlanningAgent; cwd: string; dependencies: readonly AgentTask[] }>;
...
export class AgentTaskExecutor {
  ...
  async execute(context: TaskExecutionContext, signal: AbortSignal, onSession: (session: AgentSessionReference) => Promise<void>, onExecutionStarted?: () => Promise<void>): Promise<TaskExecutionResult> {
    ...
    const prompt = JSON.stringify({ goal: context.run.goal, plan: context.plan.summary, task: { title: context.task.title, description: context.task.description,
